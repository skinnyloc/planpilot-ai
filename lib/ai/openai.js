...
export class OpenAIClientService {
  constructor() {
    ...
    this.apiKey = process.env.NEXT_PUBLIC_OPENAI_API_KEY || localStorage.getItem('openai_api_key');
    this.baseURL = 'https://api.openai.com/v1';

    if (!this.apiKey) {
      console.warn('OpenAI API key not found. Looking for NEXT_PUBLIC_OPENAI_API_KEY or fallback options.');
    }
  }
  ...
  getApiKey() {
    ...
    if (typeof window !== 'undefined' && window.location.hostname === 'localhost') {
      ...
      // Since Vite doesn't expose non-NEXT_PUBLIC_ prefixed variables to the client,
      // we'll need to use the server-side proxy or expose it as NEXT_PUBLIC_OPENAI_API_KEY
      return null;
    }
  ...
  async generateContent({
    ...
    if (!apiKey) {
      const error = 'OpenAI API key not found. Please set NEXT_PUBLIC_OPENAI_API_KEY in your environment or configure the key properly.';
      console.error(error);
